...
Our Final Solution is the Candidate 2 model, which demonstrated superior performance over the ensemble.
But, our ensemble model uses three main candidates:
1.  **Candidate 1:** `DeBERTa-v3-base` (fine-tuned with LoRA)
2.  **Candidate 2:** `e5-base-v2` Embeddings + `XGBoost`/`LightGBM`
3.  **Candidate 3:** `e5-base-v2` Embeddings + Strong Lexical Feature**s** + `MLP`
...
-   `/notebooks/04_ensemble_experiment.ipynb`: This file executes the various ensemble strategies... which, though they did not become the final submission.
...
3.  **Run Inference Notebook:**
    ...
    * Run all cells. The notebook will load your trained C2 model, perform calibration, and save the **final** `submission.csv`.
**